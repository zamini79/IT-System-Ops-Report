...
export async function gcpDataHandler(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const { taskId } = await startGcpDataCollection({ jobId, userId: user.sub });

    res.status(202).json({
      success: true,
      data:    { taskId, jobId },
      message: "GCP 데이터 수집(3개 리포트)이 시작되었습니다. SSE 스트림에서 진행 상태를 확인하세요.",
    });
  } catch (err) {
    next(err);
  }
