 ...
 * 페이지 구성:
 *  - 표지: "개발본부 시스템 운영 현황"
 *  - Page 1: "1. GCP Quality System 사용 현황"
 *            Systemusage_GCP + Personwithuser_Clinical 이미지
 *  - Page 2: "2. Medcomms 사용 현황"
 *            Monthly_Medcomms 이미지
 *  - Page 3 (선택): "3. Managed Service 진행 현황"
...
function loadChartJsScript(): string {
  ...
  for (const p of candidates) {
    if (fs.existsSync(p)) {
      logger.info(`[DEV Report] Chart.js 로컬 번들: ${p}`);
...
function readMsTimesheetData(xlsxPath: string): MsTimesheetData {
  ...
  for (const sheetName of monthSheets) {
    ...
    if (gmpRowIdx < 0) {
      logger.warn(`[DEV Report MS] ${sheetName}: SKB GMP 행 없음`);
      continue;
    }
    ...
    logger.info(`[DEV Report MS] ${sheetName} SKB GMP — B:${gmpRow[1]}, C:${gmpRow[2]}, D:${gmpRow[3]}`);
    ...
    if (sheetName === monthSheets[monthSheets.length - 1]) {
      ...
      logger.info(`[DEV Report MS] ${sheetName} 테이블 행: ${tableRows.length}개`);
    }
  }
...
(function() {
  ...
  try {
    ...
    logger.info(`[DEV Report MS] Chart.js 로드: ${loaded ? "성공" : "실패"}`);
    await page.locator("#wrap").screenshot({ path: outputPng, type: "png" });
    logger.info(`[DEV Report MS] Bar chart PNG: ${outputPng} (${fs.statSync(outputPng).size.toLocaleString()} bytes)`);
  } finally {
    await browser.close();
  }
}
...
function readImageBase64(filePath: string): { base64: string; mime: "image/png" | "image/jpeg" } {
...
function buildDevReportHtml(
  titleDate: string,
  images: {
    systemusageGcp:         { base64: string; mime: string } | null;
    personwithuserClinical: { base64: string; mime: string } | null;
    monthlyMedcomms:        { base64: string; mime: string } | null;
  },
  ...
  // ── Page 1 이미지 HTML ────────────────────────────────────────────────────
  const gcpImgHtml = images.systemusageGcp
    ? `<img class="img-full" src="data:${images.systemusageGcp.mime};base64,${images.systemusageGcp.base64}" alt="GCP Quality System 사용 현황" />`
    : `<div class="placeholder-box">Systemusage_GCP 파일이 없습니다.<br>먼저 업로드해주세요.</div>`;

  const clinicalImgHtml = images.personwithuserClinical
    ? `<img class="img-full" src="data:${images.personwithuserClinical.mime};base64,${images.personwithuserClinical.base64}" alt="Personwithuser Clinical" />`
    : `<div class="placeholder-box">Personwithuser_Clinical 파일이 없습니다.<br>먼저 업로드해주세요.</div>`;

  // ── Page 2 이미지 HTML ────────────────────────────────────────────────────
  const medcommsImgHtml = images.monthlyMedcomms
    ? `<img class="img-full" src="data:${images.monthlyMedcomms.mime};base64,${images.monthlyMedcomms.base64}" alt="Monthly Medcomms" />`
    : `<div class="placeholder-box">Monthly_Medcomms 파일이 없습니다.<br>먼저 업로드해주세요.</div>`;
  ...
  const msPageHtml = msData ? (() => {
    ...
    <div class="ms-section">
      ...
      <div class="ms-chart-subtitle">개발본부 Quality System Managed Service 현황</div>
...
<body>
  ...
  <!-- ── Page 1: GCP Quality System 사용 현황 ── -->
  <div class="page">
    <div class="page-header">
      <h2>1. GCP Quality System 사용 현황</h2>
      <span class="pg">${titleDate}</span>
    </div>
    <div class="img-block">
      <div class="img-title">GCP Quality System 사용 현황</div>
      ${gcpImgHtml}
    </div>
    <div class="img-block">
      <div class="img-title">Person with User (Clinical)</div>
      ${clinicalImgHtml}
    </div>
    <div class="footer">
      <span>SK Bioscience 개발본부 — 시스템 운영 현황</span>
  ...
  <!-- ── Page 2: Medcomms 사용 현황 ── -->
  <div class="page">
    <div class="page-header">
      <h2>2. Medcomms 사용 현황</h2>
      <span class="pg">${titleDate}</span>
    </div>
    <div class="img-block">
      <div class="img-title">Monthly Medcomms 현황</div>
      ${medcommsImgHtml}
    </div>
    <div class="footer">
      <span>SK Bioscience 개발본부 — 시스템 운영 현황</span>
...
export async function generateDevReport(jobId: string): Promise<DevReportResult> {
  ...
  // 1) 이미지 파일 탐색 (옵션 — 없어도 진행)
  const gcpPath       = resolveImagePath(uploadPath, "Systemusage_GCP");
  const clinicalPath  = resolveImagePath(uploadPath, "Personwithuser_Clinical");
  const medcommsPath  = resolveImagePath(uploadPath, "Monthly_Medcomms");

  logger.info(`[DEV Report] Systemusage_GCP       : ${gcpPath ?? "없음"}`);
  logger.info(`[DEV Report] Personwithuser_Clinical: ${clinicalPath ?? "없음"}`);
  logger.info(`[DEV Report] Monthly_Medcomms       : ${medcommsPath ?? "없음"}`);

  // 2) 이미지 base64 변환
  const images = {
    systemusageGcp:         gcpPath      ? readImageBase64(gcpPath)      : null,
    personwithuserClinical: clinicalPath ? readImageBase64(clinicalPath) : null,
    monthlyMedcomms:        medcommsPath ? readImageBase64(medcommsPath) : null,
  };
  ...
  try {
    ...
    if (tsRows.length && fs.existsSync(tsRows[0].stored_path)) {
      ...
      if (msData.chartRows.length > 0) {
        ...
        logger.warn("[DEV Report] Timesheet 에서 YYYY-MM 시트 데이터 없음 — MS 페이지 스킵");
        msData = null;
      }
    } else {
  ...
  const html       = buildDevReportHtml(titleDate, images, msData, msBarChartBase64);
  ...
  if (typeof AppError === "undefined") {
    // AppError 는 임포트만 함 — 필요 시 throw new AppError(code, msg)
  }
