/**
 * DEV — Medcomms (Medical Contents Management System) Veeva Vault 대시보드 스크린샷 크롤러
 *
 * 로그인 후 아래 URL의 대시보드를 캡처합니다.
 *   https://sk-medical.veevavault.com/ui/#dashboards/viewer/0DB000000008001
 *
 * 필터 설정 흐름:
...
export class DevMedcommsDashboardCrawler extends BaseCrawler {
  private static readonly LOGIN_URL     = "https://login.veevavault.com";
  private static readonly DASHBOARD_URL =
    "https://sk-medical.veevavault.com/ui/#dashboards/viewer/0DB000000008001";

  private readonly veevaUser = process.env.DEV_MEDCOMMS_VEEVA_USER ?? process.env.LHOUSE_VEEVA_USER ?? "apiadmin@sk.com";
  private readonly veevaPass = process.env.DEV_MEDCOMMS_VEEVA_PASS ?? process.env.LHOUSE_VEEVA_PASS ?? "12345QWert";
  ...
  protected async downloadReport(): Promise<string[]> {
    ...
    await this.page.goto(DevMedcommsDashboardCrawler.LOGIN_URL, {
    ...
    // ── Step 2. Vault 선택 (sk-medical.veevavault.com) ────────────────────────
    this.emit("navigating", "Medical Vault 탐색 중…", 22);
    await this.page.waitForTimeout(2_000);
    ...
    // Medical(Medcomms) vault 이름 후보 — 환경에 따라 실제 이름이 다를 수 있음
    const vaultNames = [
      "SKY Medical Production", "Medical Production",
      "Medcomms Production", "Medcomms",
      "Medical", "sk-medical",
    ];
    ...
    if (!vaultSelected) {
      this.emit("navigating", "Medical Vault 옵션 미발견 — 대시보드 URL로 직접 접속합니다.", 25);
    }

    // ── Step 3. 대시보드 URL 접속 ──────────────────────────────────────────────
    this.emit("navigating", "Medcomms 대시보드 페이지 접속 중…", 28);
    await this.page.setViewportSize({ width: 1600, height: 1000 });

    // Vault 선택 후 이미 같은 origin(sk-medical.veevavault.com/ui/) 에 와 있으면, 해시(#dashboards/...)만
    // 다른 URL로의 goto는 same-document 이동이라 net::ERR_ABORTED 가 발생한다.
    // 같은 문서면 in-page 해시 변경으로 라우팅하고, 다른 문서일 때만 goto 한다.
    const targetUrl = DevMedcommsDashboardCrawler.DASHBOARD_URL;
    const sameDoc   =
    ...
    const ts        = Date.now();
    const tempPath  = path.join(this.downloadDir, `_tmp_medcomms_dashboard_${ts}.png`);
    const savedPath = path.join(this.downloadDir, `medcomms_dashboard_${ts}.png`);
