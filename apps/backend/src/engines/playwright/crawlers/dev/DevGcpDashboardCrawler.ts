...
export class DevGcpDashboardCrawler extends BaseCrawler {
  ...
  private async _applyDateFilter(
    ...
    // ── ② 날짜 입력 ──────────────────────────────────────────────────────
    // 이 필터 팝업("Select Dashboard Values")에는 여러 개의 날짜 범위 필터가 있고,
    // 각 필터마다 from/to 입력칸이 있다(예: Performance Statistics / Quality Event /
    // Activity / Document → 총 8개 input). 따라서 "처음 2개"만 채우면 첫 필터만
    // 바뀌고 나머지는 기본값(예: 2026-02-01)이 남는다. 모든 input 을 (from=start,
    // to=end) 짝으로 채워야 한다.
    //
    // 입력칸은 네이티브 date input(YYYY-MM-DD)이므로 Playwright 의 fill(ISO) 가
    // 가장 안정적이고, 실패 시 실제 키 입력(pressSequentially)으로 폴백한다.
    // (JS 로 value 만 직접 주입하면 picker 내부 상태가 안 바뀌어 적용되지 않음)
    const isoStart = startStr.replace(/^(\d{2})\/(\d{2})\/(\d{4})$/, "$3-$1-$2");
    const isoEnd   = endStr.replace(/^(\d{2})\/(\d{2})\/(\d{4})$/, "$3-$1-$2");

    // 패널 내부의 visible/enabled 한 날짜성 input 들을 DOM 순서대로 태깅
    ...
    if (inputCount >= 2) {
      // 각 input 을 채운다. 짝수 index = from(start), 홀수 index = to(end).
      const fillOne = async (idx: number, iso: string, typed: string): Promise<void> => {
        ...
        if (type === "date") {
          // 네이티브 date input → ISO(YYYY-MM-DD) fill 이 가장 확실
      ...
      // 입력값 검증 — 모든 칸의 실제 반영값을 읽어 로그로 노출
      const values: string[] = [];
      for (let i = 0; i < inputCount; i++) {
        values.push(
    ...
    if (!continueDone) {
      // Escape 는 팝업을 "취소"하여 입력한 날짜가 폐기되므로 절대 사용하면 안 된다.
      // Enter 로 필터를 커밋한다(다른 대시보드 크롤러와 동일 동작).
  ...
  protected async downloadReport(): Promise<string[]> {
    ...
    // Vault 선택 후 이미 같은 origin(sk-gcp.veevavault.com/ui/) 에 와 있으면, 해시(#dashboards/...)만
    // 다른 URL로의 goto는 same-document 이동이라 net::ERR_ABORTED 가 발생한다.
    // 같은 문서면 in-page 해시 변경으로 라우팅하고, 다른 문서일 때만 goto 한다.
    const targetUrl = DevGcpDashboardCrawler.DASHBOARD_URL;
    const sameDoc   =
