/**
 * BIO 연구본부 — Veeva Vault R&D 대시보드 스크린샷 크롤러
 *
 * 로그인 후 아래 URL의 대시보드를 캡처합니다.
 *   https://sk-rd.veevavault.com/ui/#dashboards/viewer/0DB000000001012
 *
 * 필터 설정 흐름:
 *   "FILTERS (N)" 텍스트 우측 연필 아이콘 클릭
 *   → 팝업에서 날짜 범위 입력 (3개월 전 1일 ~ 1개월 전 마지막 날, "DD MMM YYYY")
 *   → Continue 클릭 → 차트 재로딩 대기
 *   → 상위 2행(차트 6개) 영역 크롭하여 Systemusage_RD.png 로 저장
 */
...
export class BioRdDashboardCrawler extends BaseCrawler {
  private static readonly LOGIN_URL     = "https://login.veevavault.com";
  private static readonly DASHBOARD_URL =
    "https://sk-rd.veevavault.com/ui/#dashboards/viewer/0DB000000001012";

  private readonly veevaUser = process.env.BIO_RD_VEEVA_USER ?? process.env.LHOUSE_VEEVA_USER ?? "apiadmin@sk.com";
  private readonly veevaPass = process.env.BIO_RD_VEEVA_PASS ?? process.env.LHOUSE_VEEVA_PASS ?? "12345QWert";
  ...
  protected async downloadReport(): Promise<string[]> {
    ...
    await this.page.goto(BioRdDashboardCrawler.LOGIN_URL, {
    ...
    // ── Step 2. Vault 선택 (sk-rd.veevavault.com) ──────────────────────────────
    this.emit("navigating", "BIO R&D Vault 탐색 중…", 22);
    await this.page.waitForTimeout(2_000);
    ...
    const vaultNames = [
      "SKY BIO R&D Production", "BIO R&D Production",
      "BIO R&D", "RD Production",
      "R&D", "sk-rd",
    ];
    ...
    if (!vaultSelected) {
      this.emit("navigating", "R&D Vault 옵션 미발견 — 대시보드 URL로 직접 접속합니다.", 25);
    }
    ...
    const ts        = Date.now();
    const tempPath  = path.join(this.downloadDir, `_tmp_bio_rd_dashboard_${ts}.png`);
    const savedPath = path.join(this.downloadDir, `bio_rd_dashboard_${ts}.png`);

    await this.page.screenshot({ path: tempPath, fullPage: true, type: "png" });

    // ── Step 6. 상위 2행(차트 6개) 좌표 추출 ─────────────────────────────────
